"""
commands on terminal 2 or more
...
register node..
http://localhost:5000/nodes/register (POST) body: {“nodes”:[“http://localhost:5001"]}

register node2
http://localhost:5001/nodes/register (POST) body: {“nodes”:[“http://localhost:5000"]}
"""
...
from bc.atl.flaskrun import flaskrun
...
class Blockchain(object):
    ...
    def __repr__(self):
        return json.dumps({
            ...
            'n_current_transactions': len(self.currentTransactions),
    ...
    def valid_chain(self, chain):
        ...
        while current_index < len(chain):
            ...
            print('lastBlock: {last_block}'.format(last_block=last_block))
            print('currentBlock: {block}'.format(block=block))
            print("\n\n--------------------\n")
    ...
    def resolve_conflicts(self):
        ...
        for node in neighbors:
            print('requesting for node: {node}'.format(node=node))
            response = self.requests.get('http://{node}/chain'.format(node=node))
            print('node {node} - chain: {response}'.format(node=node, response=response))
